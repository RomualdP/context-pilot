...
//! ## How it works
//!
//! A thread-local span stack tracks the current call path. Each [`FlameGuard`]
...
impl Drop for FlameGuard {
    fn drop(&mut self) {
        ...
        SPAN_STACK.with(|s| {
            ...
            // Collect into a Vec first to join — avoids repeated String allocation.
...
/// ```ignore
/// let _fg = crate::flame!("render");
/// let _fg = crate::flame!(&format!("tool_{}", name));
/// ```
#[macro_export]
macro_rules! flame {
    ($name:expr) => {
        $crate::infra::flame::FlameGuard::new($name)
    };
}
